...
const useSetMarker = () => {
  const {map,storeList,oldMarker,oldOverlay} =useSelector(state => ({
    ...
    storeList: state.travelMap.storeList,
  ...
  const setMarker = () => {
    ...
    stores.map(value => {
      ...
      kakao.maps.event.addListener(marker, 'click', function () {
        ...
        if (Map.getLevel() >= 3) {
          Map.setLevel(3);
          Map.panTo(new kakao.maps.LatLng(value.lat + 0.0010, value.lng));
        } else if (Map.getLevel() === 2) {
          Map.panTo(new kakao.maps.LatLng(value.lat + 0.0005, value.lng));
        } else {
          Map.panTo(new kakao.maps.LatLng(value.lat + 0.0003, value.lng));
        }
